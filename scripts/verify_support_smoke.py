#!/usr/bin/env python3
"""Verify one closed PixelFlasher packaged support smoke receipt."""
...
from support_smoke_contract import (  # noqa: E402
    SupportSmokeError,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not isinstance(raw, dict):
            raise SupportSmokeError("support smoke receipt must be an object")
        source = cast(dict[object, object], raw)
        if any(not isinstance(key, str) for key in source):
            raise SupportSmokeError("support smoke receipt keys must be strings")
        receipt = validate_support_smoke_receipt(
            cast(dict[str, Any], source),
            expected_platform=args.expect_platform,
            expected_architecture=args.expect_architecture,
        )
    except (SupportSmokeError, OSError, json.JSONDecodeError) as exc:
        print(f"Packaged support smoke verification failed: {exc}", file=sys.stderr)
        return 1
