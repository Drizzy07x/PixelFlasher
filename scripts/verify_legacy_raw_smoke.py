#!/usr/bin/env python3
"""Verify one closed packaged Legacy Raw smoke receipt."""
...
from legacy_raw_smoke_contract import (  # noqa: E402
    LegacyRawSmokeError,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        receipt = validate_legacy_raw_smoke_receipt(
            load_legacy_raw_smoke_receipt(args.report),
            expected_platform=args.expect_platform,
            expected_architecture=args.expect_architecture,
        )
    except LegacyRawSmokeError as exc:
        print(f"Legacy Raw smoke verification failed: {exc}", file=sys.stderr)
        return 1
