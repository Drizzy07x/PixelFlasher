...
def _build_diagnostics(frame: wx.Frame) -> str:
    ...
    lines = [
        ...
        f"Executable: {_redact_home_path(sys.executable)}",
        ...
        f"ADB path: {_redact_home_path(shutil.which('adb') or 'not found')}",
        f"Fastboot path: {_redact_home_path(shutil.which('fastboot') or 'not found')}",
