...
| G30 | `root.pif` (`partial`, PI Analysis, inventario y eliminación verificada entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La eliminación de perfiles acepta únicamente once IDs canónicos, exige `DELETE PIF <profile> <serial-6>`, atraviesa el lock destructivo y solo finaliza cuando un observador root independiente comprueba que el archivo allow-listed está ausente. React presenta los DTO y la confirmación reforzada en los seis idiomas. Pendiente: importación/edición PIF, TargetedFix add/delete/edit/push, cleanup/check y smokes rooted empaquetados. | Cubiertos argv fijo, root ausente, aliases/campos extra, orden canónico, package IDs hostiles/duplicados, frase incorrecta, inyección de ruta, estado final presente/ausente, output parcial/hostil/sobredimensionado, DTO público cerrado y axe. Faltan las demás mutaciones PIF/TargetedFix y hardware real. |
