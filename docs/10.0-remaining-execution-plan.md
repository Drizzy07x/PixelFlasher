...
| Gates abiertos | 35 (67,3 %) | 31 `partial`, 3 `missing` y 1 `blocked`. |
...
| Comandos registrados | 81 | Cada comando conservado debe tener owner, contrato, implementación y tipos Python/TypeScript concordantes. |
| Comandos expuestos e implementados | 78 (96,3 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
| Comandos registrados no expuestos | 3 | Implementarlos o retirarlos mediante una decisión de contrato revisada antes de RC1. |
...
| Gates abiertos sin prueba citada | 3 | Deben recibir pruebas de backend, contrato, React y/o E2E antes de pasar a `native`. |
...
Los 3 comandos todavía no expuestos son: `tools.adbShell`,
`tools.myTools` y `updates.check`.
...
Los tres gates sin una prueba citada en la matriz son `navigation.about`,
`support.updates_help` y `app.personal_tools`. Son deuda de caracterización prioritaria,
...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, layout persistente de toolbar entregado el 20-07-2026) | Ninguna | Entregado: apariencia, accesibilidad, Expert Mode, seguridad, mantenimiento, patch/procesamiento, tipografía monoespaciada y layout de toolbar forman parte del contrato inmutable. La toolbar conserva posición top/right/bottom/left y visibilidad separada de contexto de dispositivo, tema e idioma; migra `toolbar.tb_position` de 9.x, responde de forma adaptativa y solo aplica cambios revisionados confirmados. Familia y tamaño rechazan valores CSS inseguros y se guardan atómicamente con mirrors. Los defaults modernos de red son opt-in. Pendiente: strings/rutas especializadas y clasificación explícita de opciones incompatibles con las políticas de 10.x. | Cubiertos tipos/campos desconocidos, posiciones/booleanos inválidos, límites numéricos, familia insegura, migración legacy, mirrors, stale revision, fallo de escritura, reinicio, round-trip y recorridos RTL del layout. Faltan property tests ampliados y smokes visuales en los tres OS para el conjunto completo. |
| G33 | `navigation.settings` (`partial`, controles tipográficos React entregados el 20-07-2026) | G32 | Entregado: apariencia, idioma, accesibilidad, zoom, tipografía monoespaciada, seguridad, mantenimiento y ocho opciones expert usan un único estado confirmado por host. Logs, consolas, código y editores estructurados comparten tokens CSS de fuente/escala; las opciones expert permanecen fuera del DOM hasta confirmar Expert Mode. Pendiente: exponer rutas mediante grants y preferencias de shell restantes con validación específica. | RTL cubre disclosure, aplicación de fuente/escala, carga, actualización, error sin falso éxito, fallback mock, límites y persistencia host; además de seis idiomas y build tipado. Faltan enumeración host de fuentes, formulario completo, teclado/foco/zoom E2E empaquetado y reinicio en los tres OS. |
...
| G36 | `support.updates_help` (`partial`, Help y enlaces nativos entregados el 20-07-2026) | G32; manifiesto de updates externo | Entregado: documentación, release notes, reporte de incidencias, fuente y licencia se abren en el navegador predeterminado mediante IDs cerrados resueltos por una allow-list HTTPS del host; el WebView conserva `connect-src 'none'` y veta navegación externa. Pendiente: implementar `updates.check` manual/automático opt-in contra el manifiesto firmado externo. | Cubiertos payload/target hostil, proyección sin URL, éxito/fallo del navegador nativo, controles React y seis idiomas. Faltan manifiesto firmado/caducado/rollback, offline, opt-in, axe y smokes empaquetados. |
