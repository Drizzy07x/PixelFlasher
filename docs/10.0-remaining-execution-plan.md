...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, preferencias expert de patch/procesamiento entregadas el 20-07-2026) | Ninguna | Entregado: apariencia, accesibilidad, Expert Mode, seguridad, mantenimiento, métodos de patch, recovery legacy, staging temporal, BusyBox, low-memory, imágenes adicionales, custom ROM y Keybox Index forman parte del contrato inmutable. Se migran desde sus claves 9.x, se guardan atómicamente con mirrors y solo se aplican tras una actualización revisionada confirmada. Los defaults modernos de red son opt-in. Pendiente: strings/rutas especializadas, fuente/consola/toolbar y clasificación explícita de opciones incompatibles con las políticas de 10.x. | Cubiertos tipos/campos desconocidos, límites 1–3600, migración legacy, mirrors, stale revision, fallo de escritura, reinicio y round-trip. Faltan property tests ampliados y permisos en los tres OS para el conjunto completo. |
| G33 | `navigation.settings` (`partial`, disclosure Expert completo para flags booleanos entregado el 20-07-2026) | G32 | Entregado: apariencia, idioma, accesibilidad, zoom, seguridad, mantenimiento y ocho opciones expert usan un único estado confirmado por host. Las opciones expert permanecen fuera del DOM hasta confirmar Expert Mode, el modo mock queda aislado y los controles de red son opt-in. Pendiente: exponer strings/rutas y preferencias de shell restantes con validación específica. | RTL cubre disclosure, carga, actualización, error sin falso éxito, fallback mock, límites, controles Settings y persistencia host; además de seis idiomas y build tipado. Faltan el formulario completo, teclado/foco/zoom E2E empaquetado y reinicio en los tres OS. |
