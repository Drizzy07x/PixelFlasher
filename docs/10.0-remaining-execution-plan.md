...
| G30 | `root.pif` (`partial`, PI Analysis, inventario, eliminación y validador de importación entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La eliminación acepta únicamente once IDs canónicos, exige `DELETE PIF <profile> <serial-6>` y solo finaliza cuando un observador root comprueba la ausencia. El validador de importación aplica políticas distintas para JSON, propiedades, listas de package IDs, marcadores y texto, limita a 1 MiB y descarta el contenido tras producir tamaño/hash. Pendiente: conectar grant, escritura y hash remoto; TargetedFix add/delete/edit/push, cleanup/check y smokes rooted empaquetados. | Cubiertos argv fijo, root ausente, aliases/campos extra, orden canónico, frase incorrecta, inyección de ruta, estado final presente/ausente, JSON malformado, duplicados/IDs hostiles, UTF-8, controles, tamaño, DTO público cerrado y axe. Faltan las demás mutaciones y hardware real. |
