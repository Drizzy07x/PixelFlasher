...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, preferencias de seguridad y mantenimiento entregadas el 20-07-2026) | Ninguna | Entregado: apariencia, accesibilidad, Expert Mode, opt-in de updates, comprobaciones de disco/bootloader/hash, updates de módulos, notificaciones y timeout de arranque forman parte del contrato inmutable. Se migran desde sus claves 9.x, se guardan atómicamente con mirrors y solo se aplican tras una actualización revisionada confirmada. Los defaults modernos de red son opt-in. Pendiente: incorporar las preferencias 9.x especializadas restantes y clasificar explícitamente las opciones incompatibles con las políticas de 10.x. | Cubiertos tipos/campos desconocidos, límites 1–3600, migración legacy, mirrors, stale revision, fallo de escritura, reinicio y round-trip. Faltan property tests ampliados y permisos en los tres OS para el conjunto completo. |
| G33 | `navigation.settings` (`partial`, seguridad y mantenimiento React entregados el 20-07-2026) | G32 | Entregado: apariencia, idioma, accesibilidad, zoom, Expert Mode, seguridad de flash y mantenimiento usan un único estado confirmado por host; el toggle lateral y Settings comparten el valor canónico y el modo mock queda aislado. Los controles de red se presentan como opt-in y el timeout está acotado. Pendiente: exponer las preferencias especializadas restantes con validación y disclosure Expert. | RTL cubre carga, actualización, error sin falso éxito, fallback mock, límites, controles Settings y persistencia host; además de seis idiomas y build tipado. Faltan el formulario completo, teclado/foco/zoom E2E empaquetado y reinicio en los tres OS. |
