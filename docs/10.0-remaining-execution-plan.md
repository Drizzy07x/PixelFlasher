...
| G20 | `boot.patch` (`partial`, registro verificado conectado al runtime el 20-07-2026) | G12, G13; APKs oficiales y runners propios | Entregado: el composition root acepta un `PatchResourceRegistry` cuyo manifest, APKs, runner y soportes ya están fijados por SHA-256; el mismo `RootingService` verificado alimenta catálogo y patching, y `BootPatchService` deja de recibir siempre un registro vacío. Pendiente: empaquetar runners propios reales, obtener bajo demanda los providers oficiales y seleccionar runner por arquitectura/KMI; el superkey APatch ya permanece como grant opaco de un uso. | Pruebas prueban manifest/recursos hostiles, protocolo, los siete sabores, composición runtime, secreto APatch, cancelación y registro final. Faltan runners de producción, KMI incompatible y smokes reales/empaquetados. |
