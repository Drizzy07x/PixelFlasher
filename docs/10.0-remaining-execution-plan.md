...
| G30 | `root.pif` (`partial`, PI Analysis, perfiles PIF, TargetedFix y cleanup DG entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La importación PIF consume un grant opaco, valida por formato, limita a 1 MiB, instala mediante staging confinado y exige SHA-256 remoto independiente. TargetedFix permite añadir/eliminar paquetes e importar perfiles JSON/PROP por grant exclusivo, garantizando un solo formato activo. Cleanup DroidGuard exige `CLEANUP DG <serial-6>`, borra únicamente la caché y `dg.db*` canónicos y solo finaliza cuando un observer root comprueba su ausencia. React nunca recibe rutas ni contenido. Pendiente: editor moderno, check interactivo y smokes rooted empaquetados. | Cubiertos grants, argv fijo, root ausente, aliases/campos extra, orden canónico, frases incorrectas, IDs/rutas hostiles, duplicados, formatos ambiguos, actualización atómica, estado final presente/ausente, hash remoto, caché persistente/ausente, validadores, DTO público, seis idiomas y axe. Faltan editor, check y hardware real. |
