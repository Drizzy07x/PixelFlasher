...
| G30 | `root.pif` (`partial`, PI Analysis, perfiles PIF y add/delete TargetedFix entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La eliminación acepta únicamente once IDs canónicos y la importación consume un grant opaco, valida por formato, limita a 1 MiB, instala mediante staging confinado y exige SHA-256 remoto independiente. TargetedFix permite añadir paquetes instalados y eliminar paquetes/perfiles asociados mediante actualización atómica de `target.txt`, frases `ADD|DELETE TARGET <package> <serial-6>` y observación root posterior. React nunca recibe rutas ni contenido. Pendiente: TargetedFix edit/push de perfiles, cleanup/check y smokes rooted empaquetados. | Cubiertos grants de propósito y un uso, argv fijo, root ausente, aliases/campos extra, orden canónico, frases incorrectas, package IDs e inyección de ruta hostiles, duplicados, actualización atómica, estado final presente/ausente, hash remoto coincidente/discrepante, validadores de contenido, DTO público cerrado, seis idiomas y axe. Faltan edit/push, cleanup/check y hardware real. |
