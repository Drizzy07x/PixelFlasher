...
| G20 | `boot.patch` (`partial`, runner propio empaquetado el 20-07-2026) | G12, G13; APKs oficiales e imágenes Legacy | Entregado: el inventario observa arquitectura, kernel y KMI; el manifest v3 separa la cadena de confianza de runners de las APKs bajo demanda y declara compatibilidad no solapada; backend y React seleccionan en modo fail-closed; y el plan liga y revalida la evidencia exacta. El runner Android propio se incluye en los cinco empaquetadores, usa BusyBox por ABI fijado por SHA-256, valida el protocolo/rutas, consume APatch por stdin y cubre Magisk, APatch, KernelSU, KernelSU-Next, SukiSU y Wild_KSU LKM. Pendiente: manifests firmados de APKs oficiales, catálogo KMI de imágenes KernelSU Legacy y smokes reales/empaquetados. | Pruebas cubren manifest/recursos hostiles, distribución parcial, packagers, alias/KMI, ambigüedad, incompatibilidades, APK descargada desacoplada, protocolo, secreto APatch, cancelación y registro final. Faltan releases reales, Legacy y smokes Android/hardware. |
