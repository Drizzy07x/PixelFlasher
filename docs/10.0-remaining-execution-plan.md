...
| G20 | `boot.patch` (`partial`, selección arquitectura/KMI entregada el 20-07-2026) | G12, G13; APKs oficiales y runners propios | Entregado: el runtime compone un `PatchResourceRegistry` verificado; el inventario observa arquitectura, kernel y KMI; el manifest v2 declara compatibilidad no solapada por runner; backend y React seleccionan en modo fail-closed; y el plan liga y revalida la evidencia exacta. El mismo `RootingService` alimenta catálogo y patching y el superkey APatch permanece como grant opaco de un uso. Pendiente: empaquetar runners propios reales, obtener bajo demanda los providers oficiales y completar smokes reales/empaquetados. | Pruebas cubren manifest/recursos hostiles, alias y extracción de KMI, ambigüedad, arquitectura/KMI ausentes o incompatibles, APK incompatible, cambio posterior al plan, protocolo, siete sabores, secreto APatch, cancelación y registro final. Faltan artefactos de producción y smokes reales/empaquetados. |
