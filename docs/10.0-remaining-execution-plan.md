...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, Expert Mode host-persisted entregado el 20-07-2026) | Ninguna | Entregado: Expert Mode forma parte del contrato inmutable, se migra desde `advanced_options`, se guarda atómicamente con mirror 9.x y solo se aplica tras una actualización revisionada confirmada. Pendiente: incorporar el resto de preferencias 9.x soportadas al schema v2. | Cubiertos tipos/campos desconocidos, migración legacy, mirror, stale revision, fallo de escritura, reinicio y round-trip. Faltan property tests ampliados y permisos en los tres OS para el conjunto completo. |
| G33 | `navigation.settings` (`partial`, Expert Mode React persistente entregado el 20-07-2026) | G32 | Entregado: apariencia, idioma, accesibilidad, zoom y Expert Mode usan un único estado confirmado por host; el toggle lateral y Settings comparten el valor canónico y el modo mock queda aislado. Pendiente: exponer cada preferencia restante con validación y disclosure Expert. | RTL cubre carga, actualización, error sin falso éxito, fallback mock, control Settings y persistencia de Expert Mode; además de seis idiomas y build tipado. Faltan el formulario completo, teclado/foco/zoom E2E empaquetado y reinicio en los tres OS. |
