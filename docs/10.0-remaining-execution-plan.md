...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G14 | `flash.advanced_options` (`partial`, resolución segura de slot entregada el 19-07-2026) | `device.discovery`, G10, G11 | Entregado: ambos slots y `inactive` como intención canónica resuelta exclusivamente desde el slot activo observado por backend; estado desconocido falla cerrado y `temporaryRoot + noReboot` se rechaza como ambiguo. Pendiente: artefacto downgrade backend y tabla completa opción×modo×firmware. | Ya hay argv dorado para slot inactivo y pruebas negativas de slot desconocido/combinación incompatible; faltan artefacto downgrade, matriz completa de opciones y properties de invariantes. |
