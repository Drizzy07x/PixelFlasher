...
| Comandos expuestos e implementados | 57 (80,3 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
| Comandos registrados no expuestos | 14 | Implementarlos o retirarlos mediante una decisión de contrato revisada antes de RC1. |
...
Los 14 comandos todavía no expuestos son: `backups.delete`, `backups.list`,
`firmware.catalog.refresh`, `firmware.pick`, `firmware.pickCustomRom`,
`tools.adbShell`, `tools.dataAdb`, `tools.keybox`, `tools.myTools`,
...
| G39 | `dev.tools` (`partial`, flujos AVB downgrade y XML binario nativos entregados el 19-07-2026) | `native.file_dialogs` | Implementar keybox validation como servicio acotado y tipado; cerrar smokes AVB/XML empaquetados y hardware donde aplique. | AVB cubre SPL, fingerprint, hash, cancelación, postcondición, bundle real, grant opaco, bridge y React. XML usa parser propio sin shell con límites de bytes/strings/profundidad/elementos/atributos, grant ligado al inode, proyección pública cerrada, fuzz acotado y React. Falta keybox, fuzz de certificados/imágenes y validación empaquetada. |
