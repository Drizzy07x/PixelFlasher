...
Este documento convierte los 39 gates de publicación todavía abiertos de
...
| Gates cerrados | 13 (25,0 %) | Ya están en `native`; siguen sujetos a regresión empaquetada. |
| Gates abiertos | 39 (75,0 %) | 30 `partial`, 8 `missing` y 1 `blocked`. |
...
| Comandos registrados | 71 | Cada comando conservado debe tener owner, contrato, implementación y tipos Python/TypeScript concordantes. |
| Comandos expuestos e implementados | 55 (77,5 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
...
| Gates abiertos sin prueba citada | 7 | Deben recibir pruebas de backend, contrato, React y/o E2E antes de pasar a `native`. |
| Plataformas afectadas por cada gate abierto | 39 de 39 en las tres | Windows, macOS y Linux deben presentar la misma capacidad. |
...
Distribución de los 39 gates abiertos por riesgo:
...
Los siete gates sin una prueba citada en la matriz son `navigation.about`,
`support.updates_help`, `root.data_adb`, `root.pif`, `root.recovery_tools`,
`dev.tools` y `app.personal_tools`. Son deuda de caracterización prioritaria,
...
| G05 | `device.ota_diagnostics` (`partial`, preflight de estado entregado el 19-07-2026) | `device.discovery`, G01; fuente reproducible del runner | Entregado: status/preflight estricto y acotado, certificados y logs redacted. Pendiente: empaquetar el runner mutante con procedencia verificable, cancel/reset y observación independiente de estado idle. Sin reintentos automáticos tras mutación. | El status ya cubre argv serial-bound, revisión, estados/progreso desconocidos, duplicados, overflow y DTO cerrado. Faltan pruebas de runner/hash/arquitectura, estado previo incompatible, cancelación antes/después de mutar, timeout, desconexión y `postcondition_unverified`/`mismatch`. |
...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G14 | `flash.advanced_options` (`partial`, matriz backend en progreso el 19-07-2026) | `device.discovery`, G10, G11 | Entregado: ambos slots y `inactive` como intención canónica resuelta exclusivamente desde el slot activo observado por backend; estado desconocido falla cerrado. Factory/custom/OTA ya no pueden cruzarse con un modo incorrecto, wipe solo se admite mediante el modo wipe canónico y `temporaryRoot + noReboot` se rechaza como ambiguo. Pendiente: artefacto downgrade backend y properties completas de la matriz. | Hay argv dorado para slot inactivo y pruebas negativas de slot desconocido, modo×firmware, wipe encubierto y combinaciones incompatibles; faltan artefacto downgrade y properties de todas las opciones. |
| G15 | `flash.mode_dry_run` (`partial`, modos individuales caracterizados el 19-07-2026) | `device.discovery`, G10, G11, G14 | Entregado: factory, OTA y custom compilan el argv exacto, conservan fingerprint de ejecución estable y TTL de cinco minutos, no solicitan confirmación y ejecutan con cero procesos sin cambiar firmware ni plan canónicos. Pendiente: preview y simulación multidispositivo. | Pruebas cubren los tres tipos, igualdad de fingerprint, expiración, ausencia de interacción/proceso y preservación del estado canónico; falta batch. |
...
Esta wave no agrega gates de paridad: demuestra que los 39 gates restantes, una vez cerrados, son
