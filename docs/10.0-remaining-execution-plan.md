...
| Gates abiertos | 35 (67,3 %) | 34 `partial` y 1 `blocked`. |
...
| Comandos registrados | 91 | Cada comando conservado debe tener owner, contrato, implementación y tipos Python/TypeScript concordantes. |
| Comandos expuestos e implementados | 90 (98,9 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
| Comandos registrados no expuestos | 1 | `tools.adbShell` requiere el adapter PTY multiplataforma o una retirada revisada del contrato antes de RC1. |
...
El único comando todavía no expuesto es `tools.adbShell`.
