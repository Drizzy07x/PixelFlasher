...
| G34 | `app.preferences_shell` (`partial`, carpetas y lifecycle entregados el 20-07-2026) | G32 | Entregado: tipografía de superficies monoespaciadas sin reinicio; Configuración, Logs y Caché se abren mediante IDs backend-owned sin revelar rutas; `app.exit` responde antes de cerrar, rechaza revisión obsoleta/trabajo activo y el botón X se veta con un evento React, sin diálogo wx. Pendiente: layout configurable de toolbar y consola redacted con clear/export. | Cubiertos targets cerrados, ruta hostil, symlink/ausencia, stale revision, respuesta antes de close, cierre con operación activa y evento de veto. Faltan consola, persistencia de toolbar y smokes de chrome nativo en los tres OS. |
