...
| G10 | `firmware.process_stock` (`partial`, rollback transaccional entregado el 19-07-2026) | G08 | Entregado: diagnóstico cerrado, extracción confinada y rollback reversible de registros en memoria/SQLite, objetos content-addressed y salida extraída cuando falla la promoción. Pendiente: promover la cadena boot stock a `BootRepository`, retry manual y smokes empaquetados. | Golden factory/OTA y fallos/cancelaciones están cubiertos; carreras de revisión prueban que no quedan objetos parciales en memoria ni SQLite. Faltan BootRepository y smokes en las tres plataformas. |
