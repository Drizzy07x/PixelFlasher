...
| Gates abiertos | 35 (67,3 %) | 28 `partial`, 6 `missing` y 1 `blocked`. |
...
| Comandos registrados | 75 | Cada comando conservado debe tener owner, contrato, implementación y tipos Python/TypeScript concordantes. |
| Comandos expuestos e implementados | 67 (89,3 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
| Comandos registrados no expuestos | 8 | Implementarlos o retirarlos mediante una decisión de contrato revisada antes de RC1. |
| Gates abiertos con alguna prueba citada | 29 | La referencia existente caracteriza solo el subconjunto actual; no implica que el gate esté cerrado. |
| Gates abiertos sin prueba citada | 6 | Deben recibir pruebas de backend, contrato, React y/o E2E antes de pasar a `native`. |
...
Los 8 comandos todavía no expuestos son: `tools.adbShell`,
`tools.dataAdb`, `tools.myTools`,
`tools.piAnalysis`, `tools.pif`, `tools.shizuku`, `tools.sos` y
`updates.check`.
...
Los seis gates sin una prueba citada en la matriz son `navigation.about`,
`support.updates_help`, `root.data_adb`, `root.pif`, `root.recovery_tools`,
y `app.personal_tools`. Son deuda de caracterización prioritaria,
...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G24 | `apps.package_manager` (`partial`, acciones principales conectadas el 19-07-2026) | `device.discovery`, G01 | Entregado: listing, enable/disable, uninstall con keep-data, clear-data, force-stop, launch y un informe de permisos tipado/acotado, todos desde React mediante `apps.action` y refresh canónico solo tras mutaciones exitosas. Pendiente: export/download del APK, denylist y controles SU. | Cubiertos package IDs hostiles, permisos de un único paquete, salida sobredimensionada o malformada, DTO público cerrado, acciones React y refresh success-gated. Faltan matriz root/denylist/SU, export device→host y smokes empaquetados. |
...
| G26 | `backups.magisk` (`partial`) | `device.discovery`, G01, `native.file_dialogs` | El inventario content-addressed de imágenes raw, resultados, procedencia, restore por ID y delete transaccional ya están implementados; faltan los contratos específicos Magisk list/import/delete. | Añadir fixtures Magisk corruptos/duplicados e incompatibles y validar sus transacciones SQLite/dispositivo sin degradar las pruebas raw existentes. |
| G27 | `navigation.backups` (`partial`) | G26 | La ruta React ya presenta create, restore externo/administrado, list y delete con refresh canónico, procedencia y estados accesibles; falta integrar allí la semántica Magisk de G26. | Mantener RTL/axe, teclado/foco, terminal states, empty/error/loading y añadir el E2E Magisk sin diálogo wx. |
