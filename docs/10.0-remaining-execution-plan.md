...
Este documento convierte los 30 gates de publicación todavía abiertos de
...
Los identificadores ya cerrados se conservan dentro de sus waves para no
renumerar las dependencias históricas; no forman parte de los 30 gates abiertos.
...
| Gates cerrados | 22 (42,3 %) | Ya están en `native`; siguen sujetos a regresión empaquetada. |
| Gates abiertos | 30 (57,7 %) | Los 30 están `partial`; ya no queda ningún gate `blocked`. |
...
| Gates abiertos con alguna prueba citada | 30 | La referencia existente caracteriza solo el subconjunto actual; no implica que el gate esté cerrado. |
| Gates abiertos sin prueba citada | 0 | Se mantiene en cero al añadir capacidades o subdividir gates. |
| Plataformas afectadas por cada gate abierto | 30 de 30 en las tres | Windows, macOS y Linux deben presentar la misma capacidad. |
...
Distribución de los 30 gates abiertos por riesgo:
...
| G05 | `device.ota_diagnostics` (`partial`, fallback reproducible entregado el 24-07-2026) | `device.discovery`, G01 | Entregado: status, certificados y logs redacted; cancel/reset root-only con confirmación y postcondición `ota_idle_state`. Un runner DEX propio y neutral a la arquitectura usa la API de sistema `UpdateEngine`; su fuente Java y stubs de compilación están versionados, Google R8/D8 queda fijado por versión, tamaño y SHA-256 y el build reproduce byte a byte el DEX empaquetado. El runtime verifica el hash antes de planificar, lo pasa por el staging privado del runner, lo sube a una ruta fija, vuelve a verificarlo en el dispositivo y lo usa para el preflight inmediato, `cancel`, `reset` y la observación idle independiente. Rechaza idle, disabled, salida hostil, timeout y root/revisión cambiados. Cancelar antes de la primera mutación produce `CANCELLED`; después produce `FAILED/outcome_unknown`. Los binarios opacos legacy siguen excluidos. Pendiente: resultados remotos del build reproducible y smokes rooted en el rango Android soportado. Sin reintentos automáticos tras mutación. | Tests de artifact binding, toolchain lock, tamper, golden argv, confirmación denegada, estado incompatible, root ausente, cancelación antes/después, DTO público y observer idle están verdes. `scripts/build_ota_runner.py --check` reconstruye localmente el DEX exacto. Faltan timeout/desconexión y comportamiento real de Binder/SELinux contra el runner empaquetado en dispositivos Pixel. |
...
| G10 | `firmware.process_stock` (`native`, cerrado el 25-07-2026) | G08 | Entregado: diagnóstico cerrado, extracción confinada, promoción stock a `BootRepository` con hash de firmware/codenames y rollback coordinado de registros en memoria/SQLite, objetos content-addressed y salida extraída. Un fallo ofrece retry explícito ligado al ID del firmware seleccionado; si la selección cambia, el retry se invalida sin ejecutar. El smoke CLI del artefacto prueba procesamiento, hash canónico, promoción `init_boot`, persistencia y shutdown limpio. | Golden factory/OTA, formato boot, procedencia, retry manual y fallos/cancelaciones están cubiertos; carreras de revisión prueban que no quedan objetos firmware/boot parciales. El contrato local, el `.exe` Windows x64 real y la matriz remota completa están verdes en Windows x64/ARM64, macOS Intel/ARM, Ubuntu 22/24 y AppImage. |
...
| G16 | `flash.mode_keep_data` (`partial`, interlocks React cerrados el 20-07-2026) | G01, G10, G14; coordinador de transiciones | Entregado: preview y confirmación batch ligados al fingerprint, apertura del lifecycle en el último boundary validado y ejecución secuencial fail-fast mediante `OperationRunner.execute_batch`. El planner separa particiones bootloader/fastbootd, transiciona con `reboot fastboot|bootloader` y espera el dispositivo antes de continuar. React deshabilita targeting inactive/both sin slot A/B observado en todos los targets, downgrade sin firmware factory y temporary root sin boot parcheado/verificado; el planner conserva los rechazos autoritativos. Pendiente: smokes empaquetados/hardware. | El runner cubre éxito, fallo, cancelación antes/después de mutar, desconexión, revalidación y replan obligatorio. Golden tests cubren orden exacto desde fastboot y fastbootd, seriales, particiones, flags, downgrade y temporary root; RTL cubre prerequisites y reset seguro. Faltan smokes de hardware, slot, build y hash. |
...
| G18 | `flash.mode_ota` (`partial`, progreso streaming entregado el 20-07-2026) | G01, G10, G14; coordinador de recovery/sideload | Entregado: el planner acepta ADB, recovery o sideload y compila `reboot sideload → wait-for-sideload → sideload` sin shell. `SubprocessTransport` drena ambos pipes con límite agregado y publica durante la ejecución; un parser exclusivo de `adb sideload` recompone porcentajes fragmentados, ignora valores inválidos/regresivos y emite progreso monotónico 10–90 % sin rutas ni texto crudo del proceso. Cuando se solicita reboot final exige slot activo conocido, liga el plan al slot inactivo y verifica reconexión ADB, `boot_completed`, build y slot resultante. Pendiente: smokes empaquetados y hardware. | Pruebas cubren argv exacto desde los tres estados, streaming antes de terminar el proceso, fragmentación entre chunks, monotonía, redacción, aislamiento de comandos no OTA, presentación React, firmware/hash incompatibles, slot desconocido, reboot/no-reboot y postcondiciones tipadas. El runner convierte timeout/desconexión tras transición en `outcome_unknown`; falta validación empaquetada/hardware. |
...
| G35 | `support.package` (`partial`, smoke criptográfico empaquetado entregado el 20-07-2026) | `native.file_dialogs`; clave RSA externa | Entregado: grants de destino de un uso, SQLite saneado, allow-list, límites, redacción obligatoria, manifiesto/hash y escritura atómica v2. El smoke cerrado genera una clave receptora efímera, escribe y relee AES-256-GCM con RSA-OAEP-SHA256, comprueba dos entradas con secretos sembrados, rechaza un bit alterado, lee un fixture v1 acotado y emite un recibo sin rutas. Está conectado a Windows x64/ARM64, macOS Intel/ARM, Ubuntu 22/24 y AppImage. Pendiente: provisionar la clave receptora de producción y confirmar la matriz remota. | Cubiertos clave errónea, archivo enorme, symlink, secretos, SQLite, manifiesto/hash, ausencia de subida, v1/v2 y tamper. El `.exe` Windows x64 reconstruido completa y valida el recibo; faltan clave/rotación de producción y resultados remotos Windows ARM64, macOS y Linux. |
...
| G37 | `navigation.about` (`native`, paridad cerrada el 20-07-2026) | Ninguna; G36 conserva solo la provisión externa del manifiesto | Entregado dentro de Settings para conservar las nueve áreas: versión anunciada por el host, canal de compilación estable/RC/desarrollo, licencia, estado/canal/última versión del resultado autenticado de `updates.check`, documentación, notas de versión, proyecto y soporte. React valida el DTO cerrado completo antes de mostrarlo; un resultado offline, fallido o alterado nunca se presenta como vigente. La versión se deriva del tag en el empaquetado, no se recompila en React, y ninguna URL cruza el bridge. | Cubiertos tags stable/RC, versión de desarrollo, actualización vigente/disponible, offline, payload inválido, SemVer, cinco targets allow-listed, teclado, axe, seis idiomas, typecheck y build WebView. La provisión de claves/manifiesto y los smokes generales de artefactos permanecen en G36 y en los gates RC, sin reducir la paridad de About. |
...
| G40 | `app.personal_tools` (`native`, cerrado el 25-07-2026) | G32, `native.file_dialogs` | Entregado: perfiles `safeArgv` con grant y hash, más Legacy Raw aislado en Expert Mode. La migración 9.x conserva el comando solo en el backend; un fingerprint liga el permiso persistente y las frases `ALLOW RAW <hash-8>` / `RUN RAW <hash-8>`. React muestra el comando exacto, permite revocar y nunca envía comando, cwd ni entorno. El runner usa el shell nativo como argv bajo `shell=False`, restringe cwd a raíces aprobadas, no acepta personalización de entorno desde la UI y bloquea redirección y elevación; timeout, salida y resultado son acotados. El smoke schema v2 crea un perfil seguro mediante grant, fija hash, recarga el repositorio y ejecuta `whoami /user` o `id -u` como argv directo; después prueba por separado el shell Legacy Raw, permiso persistente y rechazo de frases incorrectas. | Cubiertos migración/rehidratación, entrada legacy inválida, persistencia/revocación, frases incorrectas, revisión y Expert Mode, redirección/elevación/cwd hostil, argv/cwd/env exactos, ruta Windows con espacios, salida distinta de cero, DTO público cerrado, bridge sin comando crudo, seis idiomas, RTL y recibo dual de proceso empaquetado. El `.exe` Windows x64 real y la matriz remota completa validan safeArgv y Legacy Raw en Windows x64/ARM64, macOS Intel/ARM, Ubuntu 22/24 y AppImage. |
...
Esta wave no agrega gates de paridad: demuestra que los 30 gates restantes, una vez cerrados, son
...
- Entregado el 20-07-2026: `--ui-smoke-report` arranca el entrypoint real,
  espera el `app.ready` de React sobre bridge v2, recorre las nueve áreas con
  `Alt+1…9`, comprueba ruta activa, foco en cada encabezado y persistencia del
  mismo documento, apaga runtime/MainLoop y solo entonces publica atómicamente
  un recibo JSON schema v2 cerrado. El mismo verificador
...
El contrato ejecutable de decisión y el formato de evidencia están documentados
en [`rc1-preflight.md`](rc1-preflight.md). Un candidato incompleto debe terminar
con código de salida distinto de cero; no existen excepciones manuales dentro
del preflight.
...
| Activo | Estado |
|---|---|
| Claves Ed25519 fijadas en `artifact_trust.py` | Provisionado: las seis familias tienen clave. |
| Catálogo de Platform Tools | Provisionado: 5 targets, verificación estricta en verde. |
| Catálogo de root-apps | Provisionado: 13 targets provider/arquitectura. |
| Catálogo de Scrcpy | Provisionado: 5 targets. Windows ARM64 fija el archivo oficial win64 porque scrcpy no publica binario ARM64; el `source-lock` lo declara, igual que hace Platform Tools. |
| Clave RSA receptora de Support | Provisionada. |
| Snapshot de revocaciones de keybox | Provisionado desde la lista de estado de atestación de Google. La ventana de validez está limitada a 31 días, así que hay que regenerarlo cada mes con `scripts/build_keybox_revocations.py`. |
| Catálogo de firmware factory/OTA | **Pendiente.** El índice de imágenes de Google exige aceptar su licencia antes de exponer las URL, y esa aceptación es una decisión del responsable de release, no del proceso de build. |
| Manifiesto de updates | **Pendiente.** Exige una `releaseUrl` real bajo el repositorio de releases; no existe ninguna publicación 10.x que firmar. |

Las claves privadas viven fuera del árbol y nunca se versionan. Las actuales se
generaron en una estación de trabajo, lo que basta para cerrar la cadena técnica
y los smokes, pero una release pública requiere custodia offline o HSM.
