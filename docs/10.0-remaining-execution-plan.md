...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, política exhaustiva 9.x entregada el 20-07-2026) | Ninguna | Entregado: apariencia, accesibilidad, Expert Mode, seguridad, mantenimiento, patch/procesamiento, tipografía monoespaciada y layout de toolbar forman parte del contrato inmutable. Un registro cerrado clasifica las 31 opciones de `advanced_settings.py` como migradas, reemplazadas por un flujo seguro o impuestas por política; ninguna se retira silenciosamente. `create_boot_tar` se migra y gobierna la salida Odin verificada. La toolbar conserva posición y visibilidad separada; los valores se guardan atómicamente con mirrors 9.x. Los defaults modernos de red son opt-in. Pendiente: property tests ampliados y smokes visuales/empaquetados en los tres OS. | Cubiertos cobertura AST exacta de las 31 claves, tipos/campos desconocidos, posiciones/booleanos inválidos, límites numéricos, familia insegura, migración/mirrors, stale revision, fallo de escritura, reinicio, round-trip, `boot.tar` y recorridos RTL. Faltan property tests ampliados y smokes visuales en los tres OS para el conjunto completo. |
| G33 | `navigation.settings` (`partial`, formulario y política 9.x completos el 20-07-2026) | G32 | Entregado: apariencia, idioma, accesibilidad, zoom, tipografía monoespaciada, seguridad, mantenimiento y nueve opciones expert usan un único estado confirmado por host. Logs, consolas, código y editores estructurados comparten tokens CSS de fuente/escala; las opciones expert permanecen fuera del DOM hasta confirmar Expert Mode. Las rutas se resuelven mediante acciones backend-owned y las preferencias de shell incompatibles quedan reemplazadas por PTY/My Tools según la política cerrada. Pendiente: teclado/foco/zoom E2E empaquetado, enumeración opcional de fuentes del host y reinicio visual en los tres OS. | RTL cubre disclosure, `createBootTar`, aplicación de fuente/escala, carga, actualización, error sin falso éxito, fallback mock, límites y persistencia host; además de seis idiomas y build tipado. Faltan smokes empaquetados multiplataforma. |
