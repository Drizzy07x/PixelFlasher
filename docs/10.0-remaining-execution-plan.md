...
| G08 | `firmware.select_stock` (`partial`, política criptográfica entregada el 20-07-2026) | `native.file_dialogs`; G07 para selección oficial | Entregado: identificación factory/full OTA, procedencia `official`/`user_supplied`, verificación streaming del formato de firma whole-file de AOSP, rechazo irrevocable de firma presente inválida, autenticación por manifiesto oficial Ed25519 o firmante OTA confiable y confirmación reforzada `TRUST FIRMWARE <sha-8>` para paquetes no reconocidos. El recibo cerrado de confianza/hash/archivo/compatibilidad se persiste en el repositorio y se muestra en React; cualquier cambio de revisión durante la confirmación aborta antes de importar. Pendiente: retry manual, roots de firmantes de producción y smokes empaquetados. | Cubierto factory/OTA firmado, firmante conocido/desconocido, unsigned, tamper, footer/EOCD/CMS hostil, fuzz de bytes arbitrarios, cancelación, revisión cambiante, persistencia y DTO/React cerrados. Faltan fixtures reales de producción y smokes de artefactos empaquetados. |
