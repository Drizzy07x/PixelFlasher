...
| Comandos expuestos e implementados | 77 (95,1 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
| Comandos registrados no expuestos | 4 | Implementarlos o retirarlos mediante una decisión de contrato revisada antes de RC1. |
...
Los 4 comandos todavía no expuestos son: `tools.adbShell`,
`tools.myTools`, `tools.pif` y `updates.check`.
...
| G30 | `root.pif` (`partial`, PI Analysis e inventario PIF/TargetedFix entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. El inventario cubre once perfiles canónicos y targets validados, pero publica solo IDs, formatos, presencia, tamaños acotados y hashes verificados; nunca rutas, contenido ni keybox. Ambos parsers exigen fronteras schema/complete, registros cerrados y evidencia completa; React presenta los DTO seguros en los seis idiomas. Pendiente: edición/push PIF, TargetedFix add/delete/edit/push, cleanup/check y smokes rooted empaquetados. | Cubiertos argv fijo, root ausente, aliases/campos extra, orden canónico, package IDs hostiles/duplicados, output parcial/hostil/sobredimensionado, fingerprint keybox prohibido, DTO público cerrado, ausencia de rutas/serial y axe. Faltan las mutaciones PIF/TargetedFix y hardware real. |
