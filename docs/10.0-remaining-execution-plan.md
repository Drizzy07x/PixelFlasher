...
| G08 | `firmware.select_stock` (`partial`, política criptográfica y retry manual entregados el 20-07-2026) | `native.file_dialogs`; G07 para selección oficial | Entregado: identificación factory/full OTA, procedencia `official`/`user_supplied`, verificación streaming del formato de firma whole-file de AOSP, rechazo irrevocable de firma presente inválida, autenticación por manifiesto oficial Ed25519 o firmante OTA confiable y confirmación reforzada `TRUST FIRMWARE <sha-8>` para paquetes no reconocidos. El recibo cerrado de confianza/hash/archivo/compatibilidad se persiste en el repositorio y se muestra en React; cualquier cambio de revisión durante la confirmación aborta antes de importar. React ofrece retry solo tras una acción explícita, repite únicamente el ID opaco o reabre el selector nativo y no reintenta cancelaciones. Pendiente: roots de firmantes de producción y smokes empaquetados. | Cubierto factory/OTA firmado, firmante conocido/desconocido, unsigned, tamper, footer/EOCD/CMS hostil, fuzz de bytes arbitrarios, cancelación, revisión cambiante, persistencia, retry explícito sin rutas y DTO/React cerrados. Faltan fixtures reales de producción y smokes de artefactos empaquetados. |
...
| G10 | `firmware.process_stock` (`partial`, cadena transaccional y retry manual entregados el 20-07-2026) | G08 | Entregado: diagnóstico cerrado, extracción confinada, promoción stock a `BootRepository` con hash de firmware/codenames y rollback coordinado de registros en memoria/SQLite, objetos content-addressed y salida extraída. Un fallo ofrece retry explícito ligado al ID del firmware seleccionado; si la selección cambia, el retry se invalida sin ejecutar. Pendiente: smokes empaquetados. | Golden factory/OTA, formato boot, procedencia, retry manual y fallos/cancelaciones están cubiertos; carreras de revisión prueban que no quedan objetos firmware/boot parciales. Faltan smokes en las tres plataformas. |
