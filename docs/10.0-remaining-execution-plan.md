...
| G12 | `boot.inventory` (`partial`) | G10 | La procedencia stock ya es visible y enlaza firmware, dispositivo y partición. Pendiente: definir y aplicar borrado seguro de metadata considerando referencias, selección actual, planes in-flight, objetos compartidos y rollback. | Pruebas SQLite/migración, deduplicación SHA-256, selección revisionada y rollback stock están cubiertas; faltan delete compartido/in-flight/canónico y recuperación tras fallo. |
