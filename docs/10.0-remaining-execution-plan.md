...
| Comandos expuestos e implementados | 76 (95,0 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
...
| G30 | `root.pif` (`partial`, PI Analysis redacted entregado el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis usa un único script backend-owned, serial/revision-bound y de solo lectura. Recopila únicamente versiones, estados, conteos y hashes permitidos; no lee certificados keybox, Android IDs, nombres de targets, configuraciones crudas ni logcat. El parser exige fronteras schema/complete, registros cerrados y evidencia completa; React presenta el DTO redacted en los seis idiomas. Pendiente: inventario/edición/push PIF, TargetedFix add/delete/edit/push, cleanup/check y smokes rooted empaquetados. | Cubiertos argv fijo, root ausente, aliases/campos extra, output parcial/hostil/duplicado/sobredimensionado, fingerprint keybox prohibido, DTO público cerrado, ausencia de rutas/serial y RTL/axe. Faltan las mutaciones PIF/TargetedFix y hardware real. |
