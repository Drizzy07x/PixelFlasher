...
| G30 | `root.pif` (`partial`, PI Analysis, perfiles PIF y gestión TargetedFix entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La importación PIF consume un grant opaco, valida por formato, limita a 1 MiB, instala mediante staging confinado y exige SHA-256 remoto independiente. TargetedFix permite añadir/eliminar paquetes con actualización atómica de `target.txt` e importar perfiles JSON/PROP por grant de propósito exclusivo. La instalación exige que el paquete sea un target, elimina el formato alternativo y solo finaliza cuando el observer prueba un único archivo activo con el hash esperado. React nunca recibe rutas ni contenido. Pendiente: editor moderno, cleanup/check y smokes rooted empaquetados. | Cubiertos grants de propósito y un uso, argv fijo, root ausente, aliases/campos extra, orden canónico, frases incorrectas, package IDs e inyección de ruta hostiles, duplicados, formatos ambiguos, actualización atómica, estado final presente/ausente, hash remoto coincidente/discrepante, validadores de contenido, DTO público cerrado, seis idiomas y axe. Faltan editor, cleanup/check y hardware real. |
