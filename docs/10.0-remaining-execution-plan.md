...
| G30 | `root.pif` (`partial`, editor seguro, PI Analysis, perfiles PIF, TargetedFix, cleanup DG y check interactivo entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La importación PIF consume un grant opaco, valida por formato, limita a 1 MiB, instala mediante staging confinado y exige SHA-256 remoto independiente. El editor carga bajo demanda uno de ocho perfiles canónicos, limita el documento a 32 KiB, conserva las rutas en backend y guarda mediante staging privado, confirmación `SAVE PIF`, compare-and-swap del hash base y postcondición SHA-256. La base headless de transformaciones/favoritos ya normaliza JSON/PROP, First API y FrameworkPatcher con límites, hashes, persistencia atómica y migración 9.x idempotente; aún falta conectarla al bridge y React. TargetedFix permite añadir/eliminar paquetes e importar perfiles JSON/PROP por grant exclusivo, garantizando un solo formato activo. Cleanup DroidGuard exige `CLEANUP DG <serial-6>`, borra únicamente la caché y `dg.db*` canónicos y solo finaliza cuando un observer root comprueba su ausencia. El check interactivo abre exclusivamente PIAC, SPIC, AIC o Play Store mediante argv fijo, confirmación ligada al serial y prueba independiente del proceso. Pendiente: integración UI de transformaciones/favoritos y smokes rooted empaquetados. | Cubiertos grants, argv fijo, allow-list de checkers y perfiles editables, root ausente, aliases/campos extra, orden canónico, frases incorrectas, IDs/rutas hostiles, documentos corruptos/sobredimensionados, compare-and-swap, duplicados, formatos ambiguos, actualización atómica, estado final presente/ausente/running, hash remoto, caché persistente/ausente, validadores, DTO público, seis idiomas y axe. Faltan integración bridge/React de transformaciones/favoritos y hardware real. |
