...
| G30 | `root.pif` (`partial`, PI Analysis, inventario, eliminación e importación verificada entregados el 20-07-2026) | `device.discovery`, G01 | Entregado: PI Analysis e inventario usan scripts backend-owned, serial/revision-bound y de solo lectura. La eliminación acepta únicamente once IDs canónicos, exige `DELETE PIF <profile> <serial-6>` y solo finaliza cuando un observador root comprueba la ausencia. La importación consume un grant opaco ligado al propósito, aplica políticas específicas para JSON, propiedades, listas de package IDs, marcadores y texto, limita a 1 MiB, instala mediante staging confinado y exige que un observador independiente confirme el SHA-256 remoto. React nunca recibe rutas ni contenido. Pendiente: TargetedFix add/delete/edit/push, cleanup/check y smokes rooted empaquetados. | Cubiertos grants de propósito y un uso, argv fijo, root ausente, aliases/campos extra, orden canónico, frases incorrectas, inyección de ruta, estado final presente/ausente, hash remoto coincidente/discrepante, JSON malformado, duplicados/IDs hostiles, UTF-8, controles, tamaño, DTO público cerrado y axe. Faltan las demás mutaciones y hardware real. |
