...
| Comandos registrados | 78 | Cada comando conservado debe tener owner, contrato, implementación y tipos Python/TypeScript concordantes. |
| Comandos expuestos e implementados | 70 (89,7 %) | No puede quedar ningún comando `not_implemented` en el artefacto final. |
...
| G28 | `root.modules` (`partial`, refresh canónico entregado el 19-07-2026) | `device.discovery`, G01, `native.file_dialogs` | Entregado: install/enable/disable/remove vuelven a leer el inventario del dispositivo en la revisión resultante únicamente tras `SUCCESS`; `FAILED`/`CANCELLED` no inventan estado local. Pendiente: descubrimiento/update y el resto de ajustes root. | Cubiertos ZIP módulo hostil, root ausente, confirmación, timeout y postcondiciones backend; RTL demuestra refresh revisionado y ausencia de refresh/estado local tras fallo. Faltan versión/update y ajustes root. |
