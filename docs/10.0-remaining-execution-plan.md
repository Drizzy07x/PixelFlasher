...
| # | Gate y estado | Dependencias | Entrega ejecutable | Evidencia de salida |
|---|---|---|---|---|
| G32 | `settings.application` (`partial`, tipografía monoespaciada moderna entregada el 20-07-2026) | Ninguna | Entregado: apariencia, accesibilidad, Expert Mode, seguridad, mantenimiento, patch/procesamiento y tipografía monoespaciada forman parte del contrato inmutable. Familia y tamaño migran desde 9.x, rechazan valores CSS inseguros, se guardan atómicamente con mirrors y solo se aplican tras una actualización revisionada confirmada. Los defaults modernos de red son opt-in. Pendiente: strings/rutas especializadas, toolbar y clasificación explícita de opciones incompatibles con las políticas de 10.x. | Cubiertos tipos/campos desconocidos, límites numéricos, familia insegura, migración legacy, mirrors, stale revision, fallo de escritura, reinicio y round-trip. Faltan property tests ampliados y permisos/fuentes instaladas en los tres OS para el conjunto completo. |
...
| G34 | `app.preferences_shell` (`partial`, shell y consola entregados el 20-07-2026) | G32 | Entregado: tipografía de superficies monoespaciadas sin reinicio; Configuración, Logs y Caché se abren mediante IDs backend-owned sin revelar rutas; `app.exit` responde antes de cerrar, rechaza revisión obsoleta/trabajo activo y el botón X se veta con un evento React, sin diálogo wx. La consola conserva como máximo 200 mensajes públicos de progreso, se limpia en memoria y se exporta a `.txt` mediante grant de un uso, escritura atómica y verificación, sin rutas en el bridge. Pendiente: layout configurable de toolbar. | Cubiertos targets cerrados, ruta hostil, symlink/ausencia, stale revision, respuesta antes de close, cierre con operación activa, evento de veto, límites/clear/export de consola, grant ligado a propósito y publicación route-free. Faltan persistencia de toolbar y smokes de chrome nativo en los tres OS. |
