...
Este documento convierte los 40 gates de publicación todavía abiertos de
...
El identificador G04 se conserva en la Wave 1 como checkpoint cerrado para no
renumerar las dependencias históricas; no forma parte de los 40 gates abiertos.
...
| Gates cerrados | 12 (23,1 %) | Ya están en `native`; siguen sujetos a regresión empaquetada. |
| Gates abiertos | 40 (76,9 %) | 30 `partial`, 9 `missing` y 1 `blocked`. |
...
| Plataformas afectadas por cada gate abierto | 40 de 40 en las tres | Windows, macOS y Linux deben presentar la misma capacidad. |
...
Distribución de los 40 gates abiertos por riesgo:
...
Gate de la wave: G03, G05 y G06 están `native`; G04 ya está cerrado. Toda transición o proceso externo
...
Esta wave no agrega gates de paridad: demuestra que los 40 gates restantes, una vez cerrados, son
