...
| Keep-data, wipe, dry-run and OTA planning | partial | device write/destructive | React intent, verified factory/direct-image artifacts and backend enforcement exist. Dry-run is complete for factory, OTA and custom ROM, including immutable process-free multi-device previews. Inactive-slot intent is resolved only by the backend, firmware kinds cannot cross into incompatible modes, wipe cannot be smuggled through a preserve/images plan, and temporary-root/no-reboot ambiguity is rejected. Downgrade artifact production, recovery/fastbootd orchestration and remaining destructive mode coverage remain open. |
