...
| Keep-data, wipe, dry-run and OTA planning | partial | device write/destructive | React intent, verified factory/direct-image artifacts and backend enforcement exist. Inactive-slot intent is now resolved only by the backend from the observed active slot, unknown slot state fails closed, and temporary-root/no-reboot ambiguity is rejected. Downgrade artifact production, recovery/fastbootd orchestration and remaining mode coverage remain open. |
