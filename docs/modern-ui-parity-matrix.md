...
| Modern presentation preferences | partial | host write | The immutable host-backed contract covers appearance, accessibility, Expert Mode, safety, maintenance, patch/processing, typography and toolbar layout. A closed registry classifies all 31 advanced 9.x settings as migrated, safely replaced or policy-enforced; no option is silently retired. Only broader property coverage and packaged visual/accessibility smokes remain open. |
