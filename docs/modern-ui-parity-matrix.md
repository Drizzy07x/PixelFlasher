...
| Select/process factory, OTA and custom ROM packages | partial | host read/write | The React flow uses native grants and typed `firmware.select`/`firmware.process` commands. Factory/full-OTA verification now exposes a closed provenance, hash, archive and compatibility receipt. Selection and processing roll back new SQLite rows, content-addressed objects, planner registrations and extracted output if revisioned state promotion fails. BootRepository promotion, package-signature policy, retry UX, packaged smokes and custom `payload.bin` remain open. |
