...
| Logcat and file push | partial | device read/write and host write | Logcat now has bounded snapshot and real incremental stream modes, serial/revision binding, cancellation, strict/standard/Expert redaction and atomic export through a one-use native grant. Remote buffer clearing, the remaining format verbs/adverbs, regex/UID filters and React tag/priority controls remain open. File push is native: it uses opaque superseding grants, private verified staging, closed destinations, bounded per-file progress, explicit cancellation/manual retry and one batched remote SHA-256 verification with a bounded `toybox` fallback. |
...
| Device tools | Safe URL opening, independently extracted per-slot bootloader versions, the explicit expert ADB shell decision, packaged scrcpy discovery/lifecycle, remote Logcat buffer clearing, remaining Logcat format/filter controls and OTA cancel/reset. Read-only otacerts inspection, bounded Logcat snapshot/stream/export/redaction and the filtered update_engine snapshot are now native. |
