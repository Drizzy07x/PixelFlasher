...
| Logcat and file push | partial | device read/write | Logcat remains a timeout/line-bounded viewer. File push is native: it uses opaque superseding grants, private verified staging, closed destinations, serial/revision binding, bounded per-file progress, explicit cancellation/manual retry and one batched remote SHA-256 verification with a bounded `toybox` fallback. Logcat export/stream redaction remains open. |
