...
| Sanitized support package | partial | host write | Native destination grants, strict allow-listed collection, mandatory redaction, a sanitized SQLite copy, an inclusion/omission manifest and atomic AES-256-GCM output with RSA-OAEP-SHA256 key wrapping are tested. A closed packaged smoke generates an ephemeral recipient, writes and reads v2, reads a bounded v1 fixture, rejects tampering, seeds secrets and verifies a route-free receipt; Windows x64 passes locally and all six artifact targets run the same verifier. Production recipient-key injection and remote matrix results remain open. |
...
| Developer/personal tools | Production signed keybox revocation evidence and packaged validation of AVB downgrade, binary XML and local keybox remain. My Tools safe argv and the isolated Legacy Raw permission/confirmation flow are native; their schema-v2 smoke proves grant/hash/reload/direct argv plus the restricted native-shell boundary across Windows x64/ARM64, macOS Intel/ARM, Ubuntu 22/24 and AppImage. |
