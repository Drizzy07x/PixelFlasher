...
| Select/process factory, OTA and custom ROM packages | partial | host read/write | The React flow uses native grants and typed `firmware.select`/`firmware.process` commands. Factory/full-OTA verification exposes a closed provenance, hash, archive and compatibility receipt. Stock boot images are promoted into BootRepository with firmware/device provenance; selection and processing roll back new firmware/boot rows, content-addressed objects, planner registrations and extracted output if revisioned state promotion fails. Package-signature policy, retry UX, packaged smokes and custom `payload.bin` remain open. |
