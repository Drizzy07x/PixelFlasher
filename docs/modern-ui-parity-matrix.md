...
| Keep-data, wipe, dry-run and OTA planning | partial | device write/destructive | React intent, verified factory/direct-image artifacts and backend enforcement exist. Inactive-slot intent is resolved only by the backend, firmware kinds cannot cross into incompatible modes, wipe cannot be smuggled through a preserve/images plan, and temporary-root/no-reboot ambiguity is rejected. Downgrade artifact production, recovery/fastbootd orchestration and remaining mode coverage remain open. |
